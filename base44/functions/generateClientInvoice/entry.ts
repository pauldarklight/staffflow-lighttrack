...
Deno.serve(async (req) => {
  try {
    ...
    const amountExcl = daysWorked * clientRate;
    ...
    // Find active invoice template for client
    const allTemplates = await base44.asServiceRole.entities.Template.filter({ template_type: 'invoice_client', is_active: true });
    const template = allTemplates.find(t => t.language === language) || allTemplates[0];
    if (!template || !template.file_url) {
      return Response.json({ error: 'Geen actief sjabloon gevonden voor type "invoice_client"' }, { status: 404 });
    }
    ...
    const paymentDays = placement.payment_terms_client || 30;
    ...
    if (placement.client_bank_account) {
    ...
    const data = {
      ...
      client_rate: fmtCurrency(clientRate),
      client_rate_raw: clientRate,
      ...
      client_bank_account: placement.client_bank_account || '',
      client_bank_reference: placement.client_bank_reference || '',
    ...
    const safeClient = (placement.client_company_name || 'klant').replace(/\s+/g, '_');
    const fileName = `Factuur_${safeClient}_${getMonthName(invoiceMonth)}_${invoiceYear}.docx`;
